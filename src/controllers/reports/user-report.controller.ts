...
type ReportDto = {
  ...
  zoneId: number;
  customerId?: '';
  reportType: 'pdf' | 'excel' | 'html' | 'preview'; // pdf,html
};

export const userReport = async (req: Request, _res: Response, _next: NextFunction) => {
  try {
    const { role } = req.user.data;
    const { reportType, lang, zoneId } = req.body as ReportDto;
  } catch (error) {
    console.error(error);
  }
};
...
export const receiptReport = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { role } = req.user.data;
    const { reportType, lang, date, zoneId, customerId } = req.body as ReportDto;
    ...
    const headerReport = `ใบเสร็จ ${headerDate} ( ${findZone.zoneName} )`;
    ...
    // สร้างอินสแตนซ์ของ ReportsService
    const reportsService = new ReportsService();
    const htmlContent = await reportsService.handleLoadEjs('receipt/index.ejs', reportData);
